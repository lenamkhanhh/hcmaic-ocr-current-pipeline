...
from tools import build_ocr_dstext_parseq_shard0000_partition_notebooks as builder


def test_shard0000_partition_specs_are_balanced_and_assigned():
    assert builder.GLOBAL_SHARD_FRAME_COUNT == 24_930
    assert builder.PARTITION_COUNT == 3
    assert [builder.partition_expected_frame_count(index) for index in range(3)] == [8_310] * 3
...
def test_partition_notebooks_keep_parent_identity_and_postflight_contract():
    for partition_index, spec in enumerate(builder.PARTITION_SPECS):
        ...
        assert report["expected_frame_count"] == 8_310
        ...
        assert 'TARGET_SHARD_ID = "shard_0000"' in code
        assert "GLOBAL_SHARD_FRAME_COUNT = 24930" in code
