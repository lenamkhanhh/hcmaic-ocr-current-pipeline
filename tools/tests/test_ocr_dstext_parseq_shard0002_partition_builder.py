...
from tools import build_ocr_dstext_parseq_shard0002_partition_notebooks as builder


def test_shard0002_partition_specs_are_balanced_and_assigned():
    assert builder.GLOBAL_SHARD_FRAME_COUNT == 24_926
    assert builder.PARTITION_COUNT == 3
    assert [builder.partition_expected_frame_count(index) for index in range(3)] == [8_309, 8_309, 8_308]
...
def test_partition_notebooks_keep_shard2_identity_and_postflight_contract():
    expected_counts = [8_309, 8_309, 8_308]
    for partition_index, spec in enumerate(builder.PARTITION_SPECS):
        ...
        assert report["expected_frame_count"] == expected_counts[partition_index]
        ...
        assert 'TARGET_SHARD_ID = "shard_0002"' in code
        assert "GLOBAL_SHARD_FRAME_COUNT = 24926" in code
    ...
    # Leave canonical deliverables armed for execution after dry assertions.
